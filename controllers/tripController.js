...
//returns all trips
router.get("/", (req, res) => {
    Trip.find({}).populate('flights').populate('hotels').populate('activities').then((trip) => {
        res.json({
            status: 200,
            trip: trip,
        });
...
router.get("/:id", (req, res) => {
    Trip.findById(req.params.id).populate('flights').populate('hotels').populate('activities').then((trip) => {
        res.json({
            status: 200,
            trip: trip,
        })
